...
<hr>
	...
	<form action="updateSnack.ad" method="post"
				...
				<select name="category">
						<option value="">선택
							<c:forEach var="list" items="${ lists3 }">
								<option value="${ list.category }" <c:if test="${ list.category eq alcohol.category }">selected</c:if>>${ list.category }
							</c:forEach>
				</select>
				</td>
			</tr>
